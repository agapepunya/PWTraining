...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (document.getElementById('detail-page-content')) { // Halaman Detail Berita/Training
...
async function loadNews(limit = null) {
    ...
    itemsToDisplay.forEach(item => {
        if (item.judulberita && item.id_berita) {
            ...
            const imageUrl = item.url_gambar_1 || item.url_gambar_berita;
            const tgl = new Date(item.tanggalpublikasi).toLocaleDateString("id-ID");
            card.innerHTML = `
               <img src="${imageUrl}" alt="${item.judulberita}">
...
async function loadParticipantDetails() {
    ...
    const data = await fetchData(`getParticipantDetails&id=${participantId}`);
    if (data.error) {
...
async function loadDetailBerita(id) {
    ...
    let contentHTML = `...`; // Diisi dengan logika dari jawaban sebelumnya
    // ... (Logika lengkap untuk membangun HTML detail berita)
...
async function loadDetailTraining(id) {
    ...
    const recommendations = data.recommendations;
    let contentHTML = `...`; // Diisi dengan logika dari jawaban sebelumnya
    // ... (Logika lengkap untuk membangun HTML detail training)
    container.innerHTML = contentHTML;
    injectFooter();
}